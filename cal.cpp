...
public:
    ...
    Event(int d, int m, int y, const string& desc) 
...
void addEvent(vector<Event>& events) {
    ...
    Event event(day, month, year, description);
...
void saveEvents(const vector<Event>& events) {
    ofstream outFile("events.txt", ios::app);  // Open the file in append mode
    if (outFile.is_open()) {
        for (const auto& event : events) {
        ...
        cout << "Error opening file for saving events.\n";
    }
}
...
void loadEvents(vector<Event>& events) {
    ...
    if (inFile.is_open()) {
        ...
        while (inFile >> day >> month >> year) {
            ...
            events.push_back(Event(day, month, year, description));
        }
        inFile.close();
    }
}
